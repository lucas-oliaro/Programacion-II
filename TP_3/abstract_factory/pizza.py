...
class VeggiePizza(Pizza):
    def prepare(self):
        print(f"Preparing {self.name}")
        self.dough=self.f.create_dough(); self.sauce=self.f.create_sauce(); self.cheese=self.f.create_cheese(); self.onion=self.f.create_onion(); self.mushroom=self.f.create_mushroom()
        print(" ->", self.dough, "/", self.sauce, "/", self.cheese, "/", self.onion, "/", self.mushroom)
